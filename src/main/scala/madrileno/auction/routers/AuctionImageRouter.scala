...
class AuctionImageRouter(auctionImageService: AuctionImageService)(using TelemetryContext) extends BaseRouter {
      ...
      currentApiVersion { v =>
        complete {
          auctionImageService.listImagesWithVariants(auctionId).map[ToResponseMarshallable] { pairs =>
            Ok -> pairs.map { case (image, variants) => AuctionImageDto(image, v.urlSegment, variants) }
          }
        }
      }
    } ~
            ...
            case Some(part) =>
              ...
              auctionImageService
                ...
                .map[ToResponseMarshallable] {
                  case AttachImageResult.Attached(image) => Created -> AuctionImageDto(image, v.urlSegment)
