    ...
    (post & path("auth" / "firebase") & entity(as[AuthWithFirebaseRequest]) & pathEndOrSingleSlash & optionalHeaderValueByName(
      ...
      (
        ...
        ipAddress match {
          case Some(ip) =>
            complete {
              val command = AuthenticateWithFirebaseCommand(request.firebaseJwtToken, UserAgent(userAgent.getOrElse("Unknown")), ip)
      ...
      (post & path("auth" / "refresh-token") & entity(as[AuthWithRefreshTokenRequest]) & pathEndOrSingleSlash & optionalHeaderValueByName(
        ...
        (
          ...
          ipAddress match {
            case Some(ip) =>
              complete {
                val command = AuthenticateWithRefreshTokenCommand(request.refreshToken, UserAgent(userAgent.getOrElse("Unknown")), ip)
                authenticationService
