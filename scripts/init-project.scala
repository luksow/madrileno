...
object InitProject {
  ...
  def run(
    ...
    //     value past the rename. The underlying scripts (check-links.scala etc.) stay; forks
    //     that want the auto-check can re-add a workflow themselves.
    val templateCiWorkflows = List(
      root / ".github" / "workflows" / "link-check.yml",
      root / ".github" / "workflows" / "script-tests.yml"
    ).filter(os.exists)
    templateCiWorkflows.foreach(os.remove)
    ...
    if (templateCiWorkflows.nonEmpty) println(s"Deleted: ${templateCiWorkflows.size} template-internal CI workflows (link-check, script-tests)")
