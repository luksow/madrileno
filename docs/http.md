...
List endpoints are offset-paginated. The pieces live in `madrileno.utils.pagination` (layer-neutral — the DB DSL and the service both lean on them, the router maps to DTOs): `Limit` / `Offset` (validated opaque types), `SortDirection` (`Asc` / `Desc`), `PageRequest[F]` (limit + offset + a `sortBy: F` field-enum + direction), and the response envelope `Page[A] { items, total, limit, offset }` — a plain data type, kept codec-free; its circe `Encoder`/`Decoder` are generic `given`s on `BaseRouter`.
...
**Cursor pagination — the pieces, no endpoint yet.** Offset is right for a catalogue ("jump to page 4") but weak for a high-write feed (bid history is the candidate): deep offsets are O(offset), and concurrent inserts shift rows between pages. The keyset alternative is wired up but not yet hung on a route. The envelope is `Cursor[A] { items, hasMore }` — no `total` (the whole point of cursor is to skip the count), a separate type from `Page[A]` so neither constrains the other, and codec-free with its circe `given`s on `BaseRouter` like `Page`. The request side is `CursorRequest[K] { limit, after }` (reuses `Limit` — same 1–100, same clamp) plus the `cursorPaginated(afterSortParam, afterIdParam)` directive: `?limit=` and a two-part *raw keyset* — the sort column and the primary key as two query params (`?after-…=` + `?after-id=` by convention), supplied together or not at all (a half cursor is a 400). The token is the bare keyset, not an opaque blob — simpler, at the cost of pinning those param names into the contract.

On the DSL side a row filter mixes in `KeysetSqlFilter[S, I]` and supplies `keysetCursor`, `keysetColumns` (the `(sortColumn, pkColumn)` pair), `baseFilterFragment` (its own predicates), and optionally `keysetDescending` (defaults true / newest-first). The trait ANDs `WHERE (sortColumn, pkColumn) < (?, ?)` onto the filter, orders by `(sortColumn, pkColumn) DESC`, and fetches `limit + 1` rows; `FilteringRepository.findCursorPageByFilter(filter)` returns `(items, hasMore)` — `take(limit)` plus "was there a row beyond it", so the next-page boundary is that `+1` row, never a `COUNT(*)`. To expose cursor pagination on an endpoint: pick a feed, build a `KeysetSqlFilter`-mixing row filter (and a composite index on the two keyset columns, or the keyset `WHERE` is just a slow scan), decode the `?after-…` params into the keyset value, return `Cursor[Dto]`. If that endpoint is the bid history, mind that `bidderRef` (the per-auction pseudonym) is currently numbered from the *whole* bid list — paging it needs a stable per-bidder derivation first.
