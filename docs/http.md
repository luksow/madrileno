...
List endpoints are offset-paginated. The pieces live in `madrileno.utils.pagination` (layer-neutral — the DB DSL and the service both lean on them, the router maps to DTOs): `Limit` / `Offset` (validated opaque types), `SortDirection` (`Asc` / `Desc`), `PageRequest[F]` (limit + offset + a `sortBy: F` field-enum + direction), and the response envelope `Page[A] { items, total, limit, offset }` (`derives Encoder.AsObject, Decoder`).
