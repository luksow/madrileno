...
- **`AuctionImageRouterSpec`** — drives the router directly via `Multipart[IO]` requests with a fixed `Boundary`. Skips the baklava DSL because multipart bodies don't fit its typed body slot cleanly. The trade-off: tests run, the HTTP behavior is exercised, but the image endpoints don't appear in the generated OpenAPI / ts-rest / HTML docs under `target/baklava/`. Porting the spec to baklava once a multipart story exists is the way to get them listed.
