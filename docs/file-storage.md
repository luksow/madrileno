...
- **`AuctionImageRouterSpec`** — drives the router directly via `Multipart[IO]` requests with a fixed `Boundary`. Skips baklava DSL because multipart bodies don't compose cleanly with it; OpenAPI for these endpoints is generated from the auctions router spec via the listed routes only.

## Things that catch people out

- **The disk backend writes meta first, data second.** The sidecar `.meta` file holds the rendered `Content-Type`. `get` only returns `Streamed` if both files exist. If a crash leaves an orphan `.meta`, the next `put` overwrites it; if a crash leaves orphan data… that shouldn't happen because data is written second, but if it does, `get` returns `NotFound` because the meta is missing.
