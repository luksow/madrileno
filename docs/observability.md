...
Every log line includes the active trace-id via logback's MDC:

```
%d{"yyyy-MM-dd HH:mm:ss.SSS"} %-5level %thread %logger{36} - [traceparent: %X{traceparent}] %msg%n
```
...
1. `tc.tracer.propagate(Map.empty)` — read the current span context into a header map (traceparent, tracestate).
2. Hand that map to log4cats as the structured context.
3. log4cats writes those keys into the SLF4J MDC.
4. The MDC values land in the log line via the `%X{...}` pattern.

Three knock-on effects:

- **Console/file logs include `traceparent`.** Grep a log line, get the trace, jump straight to the trace in OpenObserve.
- **OTLP log records carry the trace context too.** OpenObserve renders logs and traces side-by-side because the trace ID is on every record.
...
`tracer.currentSpanContext` returns the active span's context (`Option[SpanContext]`) — useful when you want the trace ID for a log line or response header without doing anything else.
...
- **Console** — `traceparent` in the pattern; what you read during dev.
...
- **OpenTelemetry** — `OpenTelemetryAppender` from `opentelemetry-logback-appender-1.0`. `<captureMdcAttributes>*</captureMdcAttributes>` so MDC keys (including `traceparent`) become OTLP log attributes; `<captureExperimentalAttributes>true</captureExperimentalAttributes>` adds thread name, logger name, and similar metadata.
